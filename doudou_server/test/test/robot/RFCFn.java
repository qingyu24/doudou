...
public class RFCFn
{
	...
	{
		r.GetSendBuffer().Clear().AddID(7,5).Add(friendID).Add(giftID).Send(r.GetLink());
	}

	public static void CenterDateImpl_buyShopping(Robot r, int giftID, int price)
	{
		r.GetSendBuffer().Clear().AddID(7,6).Add(giftID).Add(price).Send(r.GetLink());
	}
	...
	{
		r.GetSendBuffer().Clear().AddID(7,7).Add(giftID).Send(r.GetLink());
	}
