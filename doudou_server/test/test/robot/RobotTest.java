...
//		FreeRun r = new FreeRun(2*60*60*1000, robotnum, rule, rule);
    ...
    private static NameRule _GetRobotNameRule(int type) {
        switch (type) {
            case 0:
                return NameRule.increaseRule;
            case 1:
                return NameRule.normalRule;
            case 2:
                return NameRule.randomRule;
            case 3:
                return NameRule.solidRule;
            default:
                return NameRule.randomRule;
        }
    }
    ...
    public static Step GetStep(int type, int stepnum) {
        switch (type) {
            case 1:
                return RFAutoMoveStep(stepnum);
            case 2:
                return RFLoginStep();
            case 3:
                return RFChatStep(stepnum);
            case 4:
                return RFAutoAllTask();
            case 5:
                return RFLog(stepnum);
            case 6:
                return RFSQLLog(stepnum);
            default:
                return null;
        }
    }
