...
public static class _ConnectionManager
{
	...
	static public void RoomImpl_chooseTeam(this ConnectionManager mgr, int teamID)
	{
		PacketBuffer.GetInstance().Clear().SetObjMethod(1,33).Add(teamID).Send(mgr);
	}
	...
	static public void CenterImpl_InvitationFriend(this ConnectionManager mgr, long p_ID, int p_teamID, long m_friendID, int ifFree)
	{
		PacketBuffer.GetInstance().Clear().SetObjMethod(3,3).Add(p_ID).Add(p_teamID).Add(m_friendID).Add(ifFree).Send(mgr);
	}
